...
import { OVERLAY_REFERENCE } from './../../modules/material/client-material.interface';
import { DnbhubTooltipComponent } from './tooltip.component';
import { TOOLTIP_DATA } from './tooltip.interface';

@Directive({
  selector: '[dnbhubTooltip]',
})
export class DnbhubTooltipDirective implements AfterContentInit, OnDestroy {
  @Input() public dnbhubTooltip?: string = void 0;

  @Input() public dnbhubTooltipDisabled = false;
  ...
  private displayTooltip(): void {
    const showTooltip = this.tooltipOnlyClamped
      ? typeof this.nativeElement !== 'undefined' &&
        this.nativeElement.scrollWidth > this.nativeElement.clientWidth
      : true;
    if (
      !this.dnbhubTooltipDisabled &&
      typeof this.dnbhubTooltip !== 'undefined' &&
      ...
      const context = Injector.create({
        providers: [
          ...
          },
          {
            ...
            useValue: {
              text: this.dnbhubTooltip,
            },
          },
        ],
      });
      const portal = new ComponentPortal(DnbhubTooltipComponent, this.viewContainerRef, context);
